        ...
        <div className="bg-wood-100 border-4 border-wood-700 p-8 shadow-[inset_2px_2px_0_rgba(255,255,255,0.45),inset_-2px_-2px_0_rgba(100,60,0,0.3),6px_6px_0_rgba(0,0,0,0.45)]">
          ...
          {status === 'success' ? (
            <div className="py-12 flex flex-col items-center text-center gap-4">
              ...
              <button
                onClick={() => { setStatus('idle'); setForm({ name: '', email: '', subject: '', category: 'question', body: '' }) }}
                className="mt-4 pixel-button px-6 py-3 cursor-pointer"
              >
                <span className="pixel-font text-wood-950" style={{ fontSize: 8 }}>SEND ANOTHER</span>
              </button>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="flex flex-col gap-6" style={{ fontFamily: 'Nunito, sans-serif' }}>
              <input
                type="text"
                name="website"
                value={website}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                  <label className={labelClass} style={{ fontSize: 7 }}>NAME</label>
                  <input
                    type="text"
                    value={form.name}
                ...
                <div>
                  <label className={labelClass} style={{ fontSize: 7 }}>EMAIL</label>
                  <input
                    type="email"
                    value={form.email}
              ...
              {/* Category */}
              <div>
                <label className={labelClass} style={{ fontSize: 7 }}>CATEGORY</label>
                <select
                  value={form.category}
              ...
              {/* Subject */}
              <div>
                <label className={labelClass} style={{ fontSize: 7 }}>SUBJECT</label>
                <input
                  type="text"
                  value={form.subject}
              ...
              <div>
                <label className={labelClass} style={{ fontSize: 7 }}>
                  MESSAGE <span className="text-wood-500" style={{ fontSize: 6 }}>(required)</span>
                </label>
                <textarea
                  value={form.body}
